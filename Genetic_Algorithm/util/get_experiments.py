...
metric = 'plausibility (FAMD GMM)'
metric_no_specs = 'FAMD GMM'
...
objectives_mean = df_real_eval_objs.mean()
...
interpolated_names = []
for stdd in stds:
    interpolated_values.append(objectives_mean + stdd*objectives_std)
    interpolated_names.append("mean_{}s".format(stdd))

# interpolated_values = [objectives_mean + 3*objectives_std]
# interpolated_names = ["mean_3s"]
...
def run_script(seed, threshold_value, plausibility_in, max_gen=10, pop=100, label=""):
    specific_args = ['--seed', str(seed), '--objective_threshold', str(threshold_value), "--plausibility", str(plausibility_in), "--max_gen", str(max_gen), "--pop", str(pop), "--label", str(label)]
    ...
    for value, name in zip(interpolated_values,interpolated_names):
        # Create a ThreadPoolExecutor
            ...
            for seed in seed_values:
                futures.append(executor.submit(run_script, seed, value, metric_no_specs, max_gen=GENERATIONS, pop=POPULATION, label=name))
            # Wait for all the scripts to complete
    for seed in seed_values:
        futures.append(executor.submit(run_script, seed, float('inf'), metric_no_specs, max_gen=GENERATIONS, pop=POPULATION))
